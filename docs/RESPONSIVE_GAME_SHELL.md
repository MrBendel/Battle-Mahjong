...
The action dock uses one horizontal row of large ceramic touch targets without a full-width ornamental container. Decorative Character/FX and the debug panel are hidden by default so they cannot reduce the Board footprint. The debug panel remains available through the `show_debug_panel` Inspector property. The Tray derives a scaled tile footprint from the Board through the shell's `tray_tile_scale` Inspector property, then reserves the corresponding rendered tile, ink outline, and queue-art height before the Board is placed. The Board ends above the action targets; region artwork and hitboxes never overlap.
...
Board tiles, tray tiles, Momentum, score, pause, and consumable actions remain readable and operable. `portrait_board_content_scale` and `landscape_board_content_scale` tune the centered puzzle footprint independently of simulation geometry; both currently default to `1.00` so the Board uses its reserved region fully. Tray tiles scale to `0.80` of the rendered Board tile footprint and transfer previews animate into that smaller target using the shell's `tile_transfer_seconds` presentation setting. When resolving a held tile compacts later tray entries, presentation previews preserve their old positions through the pair collision and then travel toward their next slot over `tray_compaction_seconds`; authoritative tray order still updates immediately.

The reference-inspired status hierarchy is shared across orientations: visible hearts, a dominant score, streak state, and a centered segmented Momentum rail. Consumables remain one compact horizontal physical cluster in portrait and at the landscape lower-left. Landscape does not stretch or rearrange the portrait-authored Board merely to consume horizontal space.
