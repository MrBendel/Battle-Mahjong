...
Board depth should be legible before the player reads individual faces. In the Default skin, vertically covered tiles retain the normal ceramic artwork with only restrained warm shading while upper tiles cast compact down-right shadows over them; overlap and vertical lift complete the depth cue. The muted front and back ceramic bases are reserved for uncovered tiles that remain locked from both sides, clearly separating gameplay lock state from physical layering. Skins without dedicated locked artwork use the cool translucent veil fallback for that side-locked state. Once a tile is selectable or otherwise visually active, it returns to the canonical enabled base at full brightness. Face-down tiles use their dedicated cosmetic back without a question mark. These treatments are presentation-only.
...
- A committed pair visually stages the incoming tile in the next open tray position, holds for a readable landing beat, collides both tile duplicates, and composes the reusable radial impact burst and pop. Undo returns or reverses the tile visual to its restored board position. When Extra Life intercepts a full tray, the final tile lands, the porcelain rack pulses red, and all four visual tiles cascade back to their stable Board slots before input resumes. Delete Pair uses the same removal primitive directly on the board. These effects never delay or mutate simulation.
- The default gameplay background uses distressed cream manga paper with black halftone, dry-ink splatters, sharp brush marks, a faded red sun, hand-drawn clouds, and restrained cyan/red edge accents. Dedicated `941 x 1672` portrait and `1672 x 941` landscape compositions keep their gameplay centers quiet; trays and HUD components provide the structural frames.
...
- The gameplay shell follows the portrait bottom dock and horizontal-reference landscape composition documented in [Responsive Game Shell](RESPONSIVE_GAME_SHELL.md).
