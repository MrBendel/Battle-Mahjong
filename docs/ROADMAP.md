...
- M11 Game Modes - in progress: Quick Play launches seeded generated boards. The session-only endless Tower is a continuous run with cumulative score and active-play time, deterministic JSON-tuned floors, and a responsive rise/smoke/drop transition between independently replayable floor definitions. Saves, rewards, checkpoints, and broader mode progression remain deferred.
