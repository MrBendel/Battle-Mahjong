...
For visual and activation review, the gameplay shell exposes `playtest_all_modifiers` in the Inspector. When enabled, the run snapshots all five level-0 modifiers and places one on the first tile of each of the opening five solver pairs. This authoring-only path is deterministic and raises that run's loadout capacity to five; the default remains off and production reference games retain the single starter Score Multiplier with normal seeded placement. Three Pair Clear uses the same skin-declared overlay contract and is represented by a circular `3` badge.
