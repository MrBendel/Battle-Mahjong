...
The Default skin's `depth_presentation` manifest section controls board-stack lighting and lift. The current Default skin uses only a restrained warm value step across authored layers; cast shadows, contact shadows, overlap, and `layer_offset_ratio` remain the primary depth cues. Other skins may map the lowest authored `z` layer to `lowest_layer_brightness` and the layer directly below the top to `near_top_layer_brightness`; intermediate layers interpolate between those anchors. The layer offset is an `[x, y]` fraction of the current rendered tile size per layer, so negative `y` moves higher layers upward and remains responsive across tile geometry and viewport sizes.

Each tile projects two presentation-only shadows: a tight, darker contact shadow derived from the active base silhouette and a larger, softer cast shadow using the skin's preblurred `shadow_asset`. The preblurred texture scales to the active orientation footprint, so the wide landscape tile does not retain a narrow portrait shadow. Both occupy the render band below their own tile surface and above the next lower tile layer. Peer tiles therefore render cleanly over one another's shadows while upper tiles visibly shade the layer beneath them. The blur is precomputed rather than sampled by a per-tile shader, keeping the effect inexpensive on mobile. Opacity, offset, and cast expansion remain skin configuration values.

Blocked state may provide a dedicated `base_variants.<orientation>.blocked_asset`. When present, the Board swaps only the ceramic base for inaccessible face-up tiles while preserving the same geometry and independent face artwork. Authored-layer lighting still applies, but the legacy cool silhouette glaze is reserved as a fallback for orientations and skins without dedicated locked artwork. A tile returns to the canonical enabled base at full brightness as soon as it is selectable or otherwise visually active. This keeps physical elevation in the shadow/depth system and availability in the ceramic material treatment. Tray tiles and moving previews always use the enabled base because they are no longer being read as part of the board stack. Face-down tiles retain their separate cosmetic back without a question mark or rectangular placeholder.
...
Tile backs have two cosmetic layers. `back_variants` supplies the revealable full-surface base, while optional `blocked_back_variants` supplies a darker inaccessible version with identical geometry. Skins or orientations without a dedicated locked back use the blocked-veil fallback. `default_back_id` then chooses an entry from `back_designs`; each entry supplies transparent ornament artwork composited over either ceramic base. `base_variants.<orientation>.back_design_safe_area` defines that ornament region. Back selection is cosmetic and must never enter matching, board, transaction, or replay logic. Durable player ownership and selection remain deferred to the profile milestone.
...
The active gameplay-background masters live at `art-source/backgrounds/gameplay_manga_paper.png` and `gameplay_manga_paper_landscape.png`; their provenance, generation prompt, and export contract live beside them. Matching runtime assets live under `game-assets/backgrounds/` and preserve the authored dimensions for the existing `48 px` scale-9 margins.
...
Default cast shadows use a tile-relative `(0.02, 0.17)` offset and `(0.06, 0.10)` expansion, with contact shadows at `(0.005, 0.045)`. This biases the depth cue downward and reduces sideways spill; all dimensions follow rendered tile size.
