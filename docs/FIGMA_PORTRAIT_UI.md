...
Optimized Godot runtime exports live under:

```text
game-assets/ui/portrait/
```

Runtime exports include the gameplay background, score box, Momentum frame/fill/badge, pause button, and composable queue cap/repeat artwork. Imported copies of the queue's blank tile and the provisional consumable-count artwork are retained but not wired into the shell: the live tray must render the same tile instances as the Board, and the consumable drawer intentionally retains its current placeholders.
...
- multiplier ticks and current multiplier;
- live tray tiles and modifiers;
- pause interaction.

The Momentum fill is clipped inside the exported frame and scales horizontally from simulation state. The queue is assembled from a left cap, one repeated section per active capacity, and a mirrored right cap. It supports capacities from two through six while the reference game remains at four.
