...
The exported pieces are resolved through `configuration/default_gameplay_theme.tres`, which is shared by portrait and landscape. The background may provide an orientation-specific composition, but HUD, tray, Board, and controls must remain independent components rather than alternate full-screen composites. A future seasonal theme can replace the background, HUD shell, tray, controls, fonts, or tile-skin manifest independently and inherit unchanged pieces from the default theme contract.
...
Runtime exports include the gameplay backgrounds, HUD top scrim, score box, Momentum frame/fill/badge, pause button, and the composable bottom action dock. Portrait retains its `941 x 1672` master dimensions and landscape uses a dedicated `1672 x 941` composition. Both preserve fixed `48 px` scale-9 margins in their source coordinate space. The `390 x 167` HUD top scrim is the exact Figma vector: a dark top-to-transparent fade that scales to the full portrait viewport width.

The shared tray is assembled from an authored porcelain left end containing the first recessed well, a repeatable complete middle well, and a right end containing the final well under `game-assets/ui/tray/porcelain/`. A tray renders `capacity - 2` middle wells, supporting two through six slots without stretching a fixed four-slot bitmap or cutting through the curved outer transitions. Portrait and landscape both use this horizontal assembly; the active orientation controls only the live tile footprint inside each well. Godot slot controls remain transparent and provide only tile/animation geometry. The untouched source composition lives at `art-source/inspiration/tiles/tile-tray.png`; run `godot --headless --path . --script res://scripts/tools/slice_porcelain_tray.gd` to regenerate source and runtime slices. The live tray must render the same tile instances as the Board.
