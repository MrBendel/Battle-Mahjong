...
- Rules version 11 requires a board mate to enter the authoritative tray before a flipped tile can resolve against it. Selecting any board tile closes the active flipped reveal, including when that tile is its mate; the player then taps the tile back to reveal and resolve against the held mate. At most one physical tile per face identity is flipped, exactly one active flipped tile may be face-up, and flipped-to-flipped direct matches are disallowed. The playable 96-tile shell exposes a default count of 12 through the Inspector.
...
- Tray tiles preserve the board's current rendered tile footprint. Natural and flipped match transfer, landing hold, two-position tray staging, collision, removal, and Undo return are presentation-only; authoritative pair resolution leaves no temporary tray occupancy.
...
- Board-stack depth is presentation-only: authored lower `z` layers are progressively darker, tile surfaces and shadows use alternating presentation bands so shadows sit below their own layer, and blocked state uses a separate cool veil. Face-down tiles use the blank ceramic base without placeholder text.
...
- Mobile background/focus loss pauses the presentation without mutating game state. Foreground recovery cancels stale touch and emulated-mouse presses, refreshes interactive controls, and requires an explicit Resume.
...
- Portrait consumables use `bottom_tray_background.png` as a horizontally stretching nine-patch: `30%` source-width margins on the left/right and `50%` source-height margins on the top/bottom. Scale the complete node uniformly to dock height so only its horizontal center span stretches. Landscape hides this artwork.
