...
- Portrait consumables recreate Figma `bottom-menu-live` (`80:30`) with exported background, ceramic caps, action icons, quantity plaques, and live Mila text in Hint, Shuffle, Delete, Undo order. `bottom_tray_background.png` stretches horizontally as a nine-patch with `30%` source-width margins and `50%` source-height margins; landscape hides all portrait action artwork and retains its side rails.
