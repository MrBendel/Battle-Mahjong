...
- M6 snapshots Hint, Undo, Delete Pair, and Shuffle quantities into each game definition. Reference runs currently start with three Hints, five Undos, one Delete Pair (shown with the bomb-style action icon), and two Shuffles. Delete Pair targets visible tiles independently from ordinary movement selectability; Shuffle preserves an almost-full tray and constructs a verified deterministic route. Persistent consumable ownership remains deferred.
...
- The Default skin uses one shared `16:23` warm-ivory front and a distinct dark-green patterned ceramic back in portrait and landscape, fifteen percent taller than the earlier `4:5` prototype. A separate replaceable back-design overlay composes over the back base. Board, tray, face-down presentation, shadows, and animation previews share the geometry; orientation only reflows peripheral UI and never changes stable layout slots or simulation rules.
...
- The M7 HUD uses runtime Mila Script Sans text, clipped Momentum fill, a square pause control, and shared compact artwork in both orientations. Portrait uses a shallow horizontal tray above its centered Board and one bottom consumable row; landscape uses a vertical tray to the Board's right and stacks the same ceramic actions at lower left. The complete portrait composition scales from its `390 x 844` reference using the limiting safe-display dimension. Full-resolution exports live under `art-source/ui/portrait/`; optimized assets live under `game-assets/ui/portrait/` and `game-assets/ui/shared/`.
