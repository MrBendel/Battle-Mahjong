...
- Build portrait and landscape gameplay UI from the same layered, themeable component tree. Orientation may change layout recipes, but do not flatten screens or duplicate complete HUD artwork; resolve replaceable visual parts through presentation-only theme resources with per-piece fallback.
...
- A PR description or comment containing `#deploy-playstore` or `#build-and-deploy` triggers `.github/workflows/deploy_playstore_hashtag.yml`. CI temporarily replaces the release export version with a monotonic 2020-epoch Play code and `<tracked-version>-internal.<code>` name, publishes to Internal testing, and syncs that release identity to Cloud Run. This does not commit or replace the per-PR tracked semantic patch bump.
...
- The playable shell opens a run-scoped modifier picker over staged gameplay chrome with Board tiles hidden. Start snapshots any distinct implemented types into a fresh `GameDefinition`, then deals the tile stack into the Board field; Restart returns to the picker with the previous choices preselected. The current unlimited debug selection expands capacity only for that run and places choices on early solver-route pairs. Persistent ownership and production loadout limits remain deferred.
- Rules version 16 makes Three Pair Clear choose a fixed activation snapshot of visible, ordinarily selectable pairs from the highest current layer downward, ranking the lower tile of each pair before its higher tile. Initially blocked pairs are never admitted. Rules version 15 and earlier retain projected lexical route ordering; Bomb remains seeded-random and may recalculate after removals.
- Rules version 17 gives Bomb a snapshotted `1..6` pair upgrade curve: level 0 clears one pair, each level adds one, and level 5 or above caps at six. The debug picker and all-modifier playtest currently equip level 4 for a five-pair activation. Progression ownership remains deferred.
- Rules version 18 records the Three Pair Clear route without atomically removing it. The shell locks player input, waits for the triggering pair to collide and fully disappear, then holds `150 ms` before submitting each planned tile through the ordinary selection path with a `50 ms` default intra-pair cadence and `550 ms` between pairs. It produces two replayable transactions and the standard queue/match presentation per pair; it must not use specialized Three Pair Clear tile animation. Rules version 17 and earlier retain atomic assisted removal for replay compatibility.
- Rules version 19 snapshots `starting_extra_life_charges` into the initial game state. Hearts share the Extra Life recovery counter: tray failure consumes one and returns unresolved tray tiles. Production profiles start at zero and must snapshot earned hearts into `GameDefinition`; the current `GameShell` supplies three only for playtesting. Rules version 18 and earlier preserve zero starting charges.
- Rules version 20 automatically reveals the final visible face-down tile on an authored layer, even when that tile remains side-locked; automatic cleanup reveals information without moving the tile. Another face-down tile on the same layer still suppresses cleanup. Cleanup reveals remain persistently face-up until selected or resolved and do not block later layer cleanup reveals. The reveal is included in the triggering transaction, records normal reveal-progress telemetry, and deterministically prefers the highest qualifying layer then physical tile ID. Rules versions 13 through 19 preserve fully manual reveals.
...
- The responsive gameplay shell uses a portrait bottom action dock and a horizontal-reference landscape composition: status at upper-left, actions at lower-left, the broad Board in the center, and shared porcelain tray sections arranged into the right rail. Only the rack artwork rotates; live tray tiles retain wide landscape geometry, upright faces, and unrotated input targets. Decorative Character/FX and debug regions yield before gameplay space.
- The Town Hub currently exposes Quick Play and Tower. Quick Play opens the seeded on-device layout picker. Tower keeps one gameplay shell for a continuous in-memory run, accumulates score and active-play time outside each independently replayable floor definition, and advances through a presentation-only rise/smoke/drop transition. Transition time is excluded from the run clock. Tower persistence, rewards, and checkpoints remain deferred.
- Tray tiles preserve the board's active geometry at a configurable uniform scale, currently `0.70`. Natural and flipped match transfer scale into the tray footprint over a configurable presentation duration. When a held match removes an interior tray tile, later tiles retain visual previews through the collision and animate toward their next horizontal or vertical slot while authoritative order updates immediately. Landing hold, two-position tray staging, collision, removal, compaction, and Undo return remain presentation-only, and authoritative pair resolution leaves no temporary tray occupancy.
...
- `UpdateChecker` calls the live Cloud Run `/v1/startup` endpoint when gameplay starts. It compares the deployed app code with the endpoint's latest code and shows the responsive update banner when behind; `version.json` and `export_presets.cfg` provide offline/local fallback metadata. Hashtag deployments sync the newly published code/name to Cloud Run automatically. No native Google Play Core plugin is currently included, so the banner falls back to opening the Play testing URL rather than performing an in-app binary update.
- The Default skin uses an upright warm-ivory front in portrait and a wide warm-ivory front in landscape, plus matching orientation-specific backs. A separate replaceable back-design overlay composes over the back base. Board, tray, face-down presentation, shadows, and animation previews share the active orientation geometry without changing stable layout slots or simulation rules.
- The six M5 modifiers use shared tile-attached artwork declared by the active skin manifest and positioned as large upper-left badges through orientation-specific modifier bounds. Board tiles, tray tiles, and moving previews render the same cosmetic overlay. Their lightweight activation presentation reuses one Board-edge/icon layer; persistent state appears beside Score for Extra Life, on Momentum for Cold Snap and Score Multiplier, and on the queue for Tray +1.
- The gameplay shell's default-off `playtest_all_modifiers` Inspector flag snapshots all six modifiers onto the opening six solver-route pairs for deterministic M7 review. Bomb is level 4 for Bomb 5, Three Pair Clear is level 2 for Match 3, and the other four remain level 0. The pregame picker exposes Bomb 1–6 and Match 1–5 and permits only one selected iteration per type. It may raise only that run's loadout capacity; production reference games retain their starter loadout and normal seeded placement.
- Three Pair Clear snapshots a level-scaled route of up to five initially selectable pairs, locks player input, and submits their tiles through the ordinary selection path on a timer; those transactions use normal Score, Momentum, Combo, difficulty, queue, and match behavior. Bomb uses gameplay RNG to recalculate and atomically remove its level-scaled route before its responsive collision chain. Both preserve unresolved modifier identities, skip identities already held in the tray, and record their ordered route in trigger telemetry.
...
- The M7 HUD uses runtime Mila Script Sans text, clipped Momentum fill, a square pause control, and shared compact artwork in both orientations. Both orientations reuse the composable horizontal porcelain tray above the Board. Portrait uses one bottom consumable row; landscape keeps the compact scorebox at upper left and stacks the same ceramic actions below it. The complete portrait composition scales from its `390 x 844` reference using the limiting safe-display dimension. Full-resolution exports live under `art-source/ui/portrait/`; optimized assets live under `game-assets/ui/portrait/` and `game-assets/ui/shared/`.
- The approved portrait proportion mask is normalized from `942 x 1672`: Status `64.9% x 9.3%`, Tray `64.9% x 9.3%`, Board `84.0% x 57.7%`, and Actions `50.0% x 11.1%`. Portrait uses full Board-region fit, `0.85` presentation-only vertical slot stride, and `0.52` tray tile scale. Landscape retains its independent `1.00` stride and `0.70` tray scale until a landscape proportion mask is approved. These settings do not change stable slots or simulation geometry.
- The portrait Status region uses an approved `613 x 155` internal grid with `7 px` gutters: hearts, score, and streak occupy the upper row; Momentum and the current multiplier occupy the lower row. Keep runtime content responsive inside these cells and do not independently recenter Momentum across the full Status region.
- The scorebox is layered and themeable: `status-panel.svg`, three live `status-heart.svg` instances, Battle Mahjong Poster Script for every label and value, an eight-stage clipped Momentum fill, and a separate flame icon. Do not bake runtime values into scorebox artwork.
- `configuration/default_gameplay_theme.tres` is the shared layered asset contract for both gameplay orientations. The shell injects it into HUD, tray, consumables, Pause, background, and tile-skin construction. Seasonal themes override individual component paths and inherit untouched defaults; never branch gameplay behavior on theme identity or replace an orientation with one flattened screen image.
...
- The consumables dock is assembled from themed left/right caps and one repeat section per visible action. Each action composes a replaceable ceramic tile and icon; exact quantity remains live Mila text in the tile's lower-right while one, two, or three vertically stacked tile layers visualize inventory counts of one, two, or three-plus. The tray and tile stack cast separate responsive shadows. Zero retains one dim disabled tile so layout never shifts as inventory is spent.
...
- Gameplay themes may provide separate portrait and landscape background compositions through the shared theme resource. Both use scale-9 rendering with fixed `48 px` source margins; landscape falls back to portrait when no override is supplied. Keep HUD, tray, Board, and controls independent from the background.
...
- Consumables reuse the approved ceramic cap and icon exports in Hint, Shuffle, Delete, Undo order without a full-width ornamental container. Portrait presents one bottom row; landscape stacks the same live controls vertically at lower left. Quantities remain runtime Mila text and disabled state remains presentation-only.
...
M9 Local Profile and Game Library remains the current cross-game architecture scope, beginning with the Town Hub application-navigation foundation. The user explicitly initiated the first M11 game-mode slice: Quick Play and a session-only endless Tower loop may evolve, but Tower persistence, rewards, checkpoints, and profile progression remain deferred. TowerRun may own session totals and floor advancement, but prior-floor values must not be injected into deterministic floor transactions. Keep hub and theme presentation independent from profile repositories and gameplay simulation. Local profile persistence, durable game records, and result application must follow the documented M9 contracts; currencies, progression, shops, social features, and backend ownership remain deferred. Do not change the 24-identity reference deal composition without an explicit gameplay decision.
...
- `docs/gameplay-ui-design.md`: gameplay-screen visual hierarchy, responsive reflow, tray ergonomics, and rejected UI directions.
...
- `docs/GCP_BACKEND_SETUP.md`: Cloud Run startup/version API, maintenance flags, and deployment operations.
