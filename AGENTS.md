...
- The gameplay shell's default-off `playtest_all_modifiers` Inspector flag snapshots all six level-0 modifiers onto the opening six solver-route pairs for deterministic M7 review. It may raise only that run's loadout capacity; production reference games retain their starter loadout and normal seeded placement.
- Three Pair Clear resolves up to three deterministic sequential pairs after its triggering pair. Bomb uses the gameplay RNG to select up to five pairs and presents them in a responsive two-column collision chain. Both recalculate selectability after every projected removal, preserve unresolved modifier identities, skip identities already held in the tray, record the actual ordered route in transaction telemetry, and give assisted pairs no Score, Momentum, Combo, difficulty reward, or recursive modifier activation.
