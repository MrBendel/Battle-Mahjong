...
- Pair collision uses one transient `PairMatchFx` node with a restrained six-particle GPU burst sharing a `64 x 64` smoke-tuft texture. The full-resolution master remains under `art-source/fx/`. It has no central cloud stamp or radial blast drawing, uses one short lifetime Tween, and performs no per-frame GDScript work; increase the effect budget only after mobile profiling.
