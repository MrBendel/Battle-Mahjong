...
- Rules version 11 requires a board mate to enter the authoritative tray before a revealed flipped tile can resolve against it. At most one physical tile per face identity is flipped, exactly one active flipped tile may be face-up, and flipped-to-flipped direct matches are disallowed. Newly uncovered tiles reveal in the board mutation transaction; tapping a flipped tile only auto-matches an existing tray mate. The playable 96-tile shell exposes a default count of 12 through the Inspector.
