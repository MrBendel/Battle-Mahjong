...
- The M7 HUD uses runtime Mila Script Sans text, a dominant score, three individually rendered prototype hearts, a centered segmented Momentum rail, a square pause control, and shared compact artwork in both orientations. Portrait uses a shallow horizontal tray above its centered Board and one bottom consumable row; landscape uses a vertical tray to the Board's right and keeps the same compact action row at lower left. The Board uses its full reserved region without changing stable slots. The complete portrait composition scales from its `390 x 844` reference using the limiting safe-display dimension. Full-resolution exports live under `art-source/ui/portrait/`; optimized assets live under `game-assets/ui/portrait/` and `game-assets/ui/shared/`.
