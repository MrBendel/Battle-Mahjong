...
func _run_tile_skin_contract_tests() -> void:
	...
	_check_equal(Vector2i(512, 672), Vector2i(skin.geometry.source_size[0], skin.geometry.source_size[1]), "skin records canonical source geometry")
	_check_equal(Vector2i(32, 42), Vector2i(skin.geometry.minimum_runtime_size[0], skin.geometry.minimum_runtime_size[1]), "skin records minimum runtime footprint")
	...
	_check_equal(str(skin.back_variants.portrait), str(skin.back_variants.landscape), "both orientations share the canonical tile-back base")
	...
	_check(is_equal_approx(portrait_aspect, 1.3125), "portrait ceramic base uses the taller canonical footprint")
...
func _run_flipped_tile_tests() -> void:
	...
	_check_equal(["flipped"], _tile_ids(direct_game.tray.tiles), "revealed flipped tile occupies an authoritative tray slot")
	_check_equal(GameStateScript.PAIR_RESOLVED, direct_game.call("tap_tile", "ordinary", 150), "ordinary mate resolves the held flipped tile normally")
	_check_equal(GameStateScript.WON, direct_game.status, "tray-played flipped tile can complete the board")

	var staged_game := GameStateScript.new(direct_definition)
	_check_equal(GameStateScript.TILE_REVEALED, staged_game.call("reveal_tile", "flipped", 100), "staged test reveals the flipped tile")
	_check_equal(GameStateScript.HINTED, staged_game.call("request_hint", 150), "Hint can use a known revealed flipped face")
	...
	_check_equal([], reversed.revealed_flipped_tile_ids, "reverse restores state before the normal matching selection")
	_check_equal(["flipped"], reversed.tray_tile_ids, "reverse restores the held flipped tile")

	var rules_eleven_definition := GameDefinitionScript.new(1, direct_tiles, {"tray_capacity": 4}, 11, [], {}, null, ["flipped"])
	var rules_eleven_game := GameStateScript.new(rules_eleven_definition)
	_check_equal(GameStateScript.SELECTED, rules_eleven_game.call("select_tile", "ordinary"), "rules version 11 stages the ordinary mate")
	_check_equal(GameStateScript.FLIPPED_PAIR_RESOLVED, rules_eleven_game.call("tap_tile", "flipped"), "rules version 11 preserves one-tap tray resolution")
	...
	_check(rules_twelve_auto_game.board.call("is_tile_revealed_flipped", "auto_flip"), "rules version 12 preserves automatic reveal for replay")
...
func _run_modifier_tests() -> void:
	...
	var clear_b_face := TileFaceScript.new("modifier", "clear_b")
	var clear_c_face := TileFaceScript.new("modifier", "clear_c")
	var chained_clear_tiles := [
		TileInstanceScript.new("trigger_a", trigger_face, BoardPositionScript.new(0, 0, 0)),
		TileInstanceScript.new("trigger_b", trigger_face, BoardPositionScript.new(24, 0, 0)),
		TileInstanceScript.new("clear_a_1", clear_a_face, BoardPositionScript.new(8, 0, 2)),
		TileInstanceScript.new("clear_a_2", clear_a_face, BoardPositionScript.new(12, 0, 2)),
		TileInstanceScript.new("clear_b_1", clear_b_face, BoardPositionScript.new(8, 0, 1)),
		TileInstanceScript.new("clear_b_2", clear_b_face, BoardPositionScript.new(12, 0, 1)),
		TileInstanceScript.new("clear_c_1", clear_c_face, BoardPositionScript.new(8, 0, 0)),
		TileInstanceScript.new("clear_c_2", clear_c_face, BoardPositionScript.new(12, 0, 0)),
	]
	var chained_clear_definition: Variant = _definition_with_modifiers(
		chained_clear_tiles,
		[clear_modifier],
		{"trigger_a": clear_modifier}
	)
	var chained_clear_game := GameStateScript.new(chained_clear_definition)
	...
	var reversed_clear: Variant = GameReducerScript.new().call(
		"apply_reverse",
		chained_clear_definition,
		chained_clear_game.call("current_snapshot"),
		clear_transaction
	)
	...
	var insufficient_game := GameStateScript.new(insufficient_definition)
