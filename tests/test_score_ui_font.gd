...
func _run() -> void:
	...
	printerr("OK: Score label font loaded: %s" % font.resource_path)
	...
	game.get("_state").score = 72590
	momentum_view.refresh(0)
	printerr("OK: Score label text after refresh(0): '%s'" % score_label.text)
	...
	game.get("_state").score = 1048576
	momentum_view.refresh(0)
	printerr("OK: Million tier score text: '%s'" % score_label.text)
	assert(score_label.text == "1,048,576", "Score label should display '1,048,576'")
	
	# Verify score update with 0
	game.get("_state").score = 0
	momentum_view.refresh(0)
	printerr("OK: Zero score text: '%s'" % score_label.text)
